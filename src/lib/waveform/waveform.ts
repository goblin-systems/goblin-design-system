...
export function cycleWaveformColorScheme(current: WaveformColorScheme): WaveformColorScheme {
...
export function getWaveformStyleLabel(style: WaveformStyle): string {
  const labels: Record<WaveformStyle, string> = {
    classic: "Classic",
    bars: "Bars",
    pulse: "Pulse",
    bloom: "Bloom",
  ...
  for (const dir of [1, -1]) {
    ...
    for (let distance = 0; distance <= maxReach; distance += 3) {
      ...
      const ripple =
        ...
        Math.sin(phase * 0.82 + progress * 18) * 0.24;
      const y = baseY - shapeAmplitude * envelope * ripple * 0.58;
      ctx.lineTo(centerX + dir * distance, y);
    }
    ctx.stroke();
...
export function drawWaveform(style: WaveformStyle, options: DrawWaveformOptions) {
  const drawers: Record<WaveformStyle, (o: DrawWaveformOptions) => void> = {
    classic: drawClassic,
    bars: drawBars,
    pulse: drawPulse,
    bloom: drawBloom,
